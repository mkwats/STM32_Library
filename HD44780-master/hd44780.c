...
LCD_RESULT LCD_Init(LCD_PCF8574_HandleTypeDef* handle) {
	...
	//mw
	LCD_StateLEDControl(handle, handle->backLight);	// LED power on
...
LCD_RESULT LCD_GetBusyFlag(LCD_PCF8574_HandleTypeDef* handle, uint8_t* flag) {
	...
	/*removed need to send these
	 * by changing toe StateSetBit
	 * then sending with the LCD enable*/
	//LCD_StateWriteBit(handle, 0, LCD_PIN_E);
	//LCD_StateWriteBit(handle, 0, LCD_PIN_RS);
	//LCD_StateWriteBit(handle, 1, LCD_PIN_RW);


	LCD_StateSetBit(handle, 0, LCD_PIN_RS);//
	LCD_StateSetBit(handle, 1, LCD_PIN_RW);//
	LCD_StateWriteBit(handle, 0, LCD_PIN_E);//
	LCD_StateWriteBit(handle, 1, LCD_PIN_E);
	...
	*flag >>= handle->pins[LCD_PIN_D7];
	...
	LCD_StateSetBit(handle, 0, LCD_PIN_E);//
	LCD_StateSetBit(handle, 0, LCD_PIN_RW);//
...
LCD_RESULT LCD_WriteCMD(LCD_PCF8574_HandleTypeDef* handle, uint8_t cmd) {
	//in 4-bit mode we first send the higher nibble and then the lower nibble

	//they use this to clear bit, but has effect of sending more data than needed
//	LCD_StateWriteBit(handle, 0, LCD_PIN_E);
//	LCD_StateWriteBit(handle, 0, LCD_PIN_RS);

	LCD_StateSetBit(handle, 0, LCD_PIN_E);
	LCD_StateSetBit(handle, 0, LCD_PIN_RS);
	//handle->state &= 0xF8; // set e. to 0  mask all but last three bits.... could be done with different pins
...
LCD_RESULT LCD_WriteDATA(LCD_PCF8574_HandleTypeDef* handle, uint8_t data) {
	...
	LCD_StateWriteBit(handle, 1, LCD_PIN_E); //rs still high
	LCD_StateWriteBit(handle, 0, LCD_PIN_E);
