...
/* improvements to send more data per address... create array then send
 * set bits in state but not write them
 * have a define or variable to store the LCD enable to mask the state....?
 * maybe have the LCD/E/RW/RS options in a array to select rather than shifting bits
 */
...
typedef struct{
	...
	uint8_t 				backLight;			/**state of backlight*/
 ...
 * @return	whether the function was successful or not
 */
LCD_RESULT LCD_StateSetBit(LCD_PCF8574_HandleTypeDef* handle, uint8_t value, LCD_PIN pin);
